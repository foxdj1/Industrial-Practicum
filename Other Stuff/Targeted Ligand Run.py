...
for i in range(2, sh.max_row+1):
    #try:
        ...
        out = openpyxl.load_workbook('C:\\Users\\fox8fv\\PycharmProjects\\Opencadd_real\\On Target Output\\' + name)
        ws = out.active
        ...
        for q in range(0, len(structure_klifs_ids)):
            coordinates = (
                KLIFS_CLIENT.Structures.get_structure_get_pdb_complex(structure_ID = structure_klifs_ids[q])
                .response()
                .result
            )
            print(coordinates)
            coordinates = eval(coordinates)
            coordinates_data = pd.DataFrame(coordinates, columns = ['number', 'atom.id', 'atom.name', 'atom.x', 'atom.y', 'atom.z', 'residue.id',
            'residue.name', 'residue.klifs_id', 'residue.klifs_region_id', 'residue.klifs_region', \
                'residue.klifs_color'])

            ws5 = out.create_sheet('Structure Coordinates' + kinase_pdb_id)

            for row in dataframe_to_rows(coordinates_data, index=True, header=True):
                ws5.append(row)
