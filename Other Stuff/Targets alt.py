...
for i in range(2, sh.max_row+1):
    try:
        species = 'Human'
        # call each specified argument from the Excel sheet then turn them into strings so the API can process them
        ...
        name = kinase_group + '_' + kinase_family + '_' + kinase_name + '_' + ligand_name + '_' + ligand_expo_id \
                            + '.xlsx'
        path = 'C:\\Users\\fox8fv\\PycharmProjects\\Opencadd_real\\On Target Output\\' +\
        ...
        subset = subset[(subset["species.klifs"] == species)]
